...
pub fn renderInstances(scene: *scene32.Scene32, entities: []entity16.Entity16, writer: anytype) !void {
    try writer.print("\nInstances:\n", .{});
    ...
    while (i < scene.instance_count) : (i += 1) {
        ...
        if (inst.entity_id < entities.len) {
            const _e = entities[inst.entity_id]; _ = _e;
