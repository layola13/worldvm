...
pub fn checkFall(scene: *scene32.Scene32, inst: *const scene32.Instance, entities: []entity16.Entity16) FallResult {
    ...
    if (below_y < 0) return .{ .can_fall = false, .target_y = 0, .blocked = true, .blocker_id = 255 };
    var ex: usize = 0;
    while (ex < 16) : (ex += 1) {
        var ez: usize = 0;
        while (ez < 16) : (ez += 1) {
            if (entity16.testVoxel(entity, @truncate(ex), 0, @truncate(ez))) {
                const sx = inst.pos_x + @as(i8, @intCast(ex));
                const sz = inst.pos_z + @as(i8, @intCast(ez));
                if (scene32.isOccupied(scene, sx, below_y, sz)) return .{ .can_fall = false, .target_y = below_y, .blocked = true, .blocker_id = 0 };
            }
        }
    }
...
fn isOccupiedByOther(scene: *scene32.Scene32, inst: *const scene32.Instance, entities: []entity16.Entity16, sx: i8, sy: i8, sz: i8) bool {
...
pub fn checkFlow(scene: *scene32.Scene32, inst: *const scene32.Instance, entities: []entity16.Entity16) FlowResult {
    ...
    for (dirs) |d| {
        ...
        if (nx < 0 or nx > 16 or ny < 0 or ny > 16 or nz < 0 or nz > 16) continue;
        var ok = true;
        for (0..16) |ex| { for (0..16) |ey| { for (0..16) |ez| {
            if (entity16.testVoxel(entity, @truncate(ex), @truncate(ey), @truncate(ez))) {
                if (isOccupiedByOther(scene, inst, entities, nx + @as(i8, @intCast(ex)), ny + @as(i8, @intCast(ey)), nz + @as(i8, @intCast(ez)))) { ok = false; break; }
            }
        } } }
...
pub fn checkPush(scene: *scene32.Scene32, inst: *const scene32.Instance, entities: []entity16.Entity16, dx: i8, dy: i8, dz: i8) PushResult {
    ...
    if (nx < 0 or nx > 16 or ny < 0 or ny > 16 or nz < 0 or nz > 16) return .{ .pushed = false, .new_x = inst.pos_x, .new_y = inst.pos_y, .new_z = inst.pos_z };
    for (0..16) |ex| { for (0..16) |ey| { for (0..16) |ez| {
        if (entity16.testVoxel(entity, @truncate(ex), @truncate(ey), @truncate(ez))) {
            if (scene32.isOccupied(scene, nx + @as(i8,@intCast(ex)), ny + @as(i8,@intCast(ey)), nz + @as(i8,@intCast(ez)))) return .{ .pushed = false, .new_x = inst.pos_x, .new_y = inst.pos_y, .new_z = inst.pos_z };
        }
    } } }
...
// Check if position is occupied by another instance (not self)
fn isOccupiedByOther(scene: *scene32.Scene32, inst: *const scene32.Instance, entities: []entity16.Entity16, sx: i8, sy: i8, sz: i8) bool {
    if (!scene32.inBounds(sx, sy, sz)) return true;
    if (!scene32.isOccupied(scene, sx, sy, sz)) return false;
    const entity = &entities[inst.entity_id];
...
// Fixed checkFall - excludes self from occupancy check
pub fn checkFallFixed(scene: *scene32.Scene32, inst: *const scene32.Instance, entities: []entity16.Entity16) FallResult {
    const entity = &entities[inst.entity_id];
    if ((entity.physics.flags & 0x01) != 0) return .{ .can_fall = false, .target_y = inst.pos_y, .blocked = false, .blocker_id = 0 };
    if (entity.physics.mass == 0) return .{ .can_fall = false, .target_y = inst.pos_y, .blocked = false, .blocker_id = 0 };
    const below_y = inst.pos_y - 1;
