...
fn dominantAxis(joint_def: *const Joint) enum { x, y, z } {
...
fn jointConstraintMagnitude(
    ...
    return switch (joint_def.joint_type) {
        ...
        .slider => blk: {
            ...
            break :blk perp + @abs(along - clamped);
        },
        .spring => blk: {
...
fn getJointAngle(inst: *const scene32.Instance, joint_def: *const Joint) f32 {
    return switch (dominantAxis(joint_def)) {
    ...
    return switch (joint_def.joint_type) {
        ...
        .slider => blk: {
            ...
            break :blk .{
                ...
                .relative_velocity =
                    @as(f32, @floatFromInt(inst_b.vel_x - inst_a.vel_x)) * axis.x +
...
pub fn solveDistanceConstraint(
    ...
    if (delta.dist <= 0.0001) return;
    recordLinearWarmStart(warm_start, delta.dx, delta.dy, delta.dz);
    applyPairCorrection(inst_a, inst_b, mass_data, delta.dx, delta.dy, delta.dz);
...
pub fn solveSpringConstraint(
    ...
    const mass_data = computeConstraintMassData(inst_a, inst_b, entities) orelse return;
    const delta = computeAnchorDelta(inst_a, inst_b, joint);
    if (delta.dist < 0.001) return;

    const rest_length = @max(0.0, joint.limit_max);
    const extension = delta.dist - rest_length;
...
fn computeJointSolvePriorityMagnitude(
    ...
    const base_stress = measureJointConstraintMetrics(inst_a, inst_b, joint_def).stress();
    if (joint_def.breaking_force <= 0.0) return base_stress;

    const break_memory_ratio = computeJointBreakRiskRatio(joint_def);
    return @max(base_stress, joint_def.breaking_force * break_memory_ratio);
}
